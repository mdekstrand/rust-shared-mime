...
fn test_text_file() {
    ...
    let answer = db.query_filename(OsString::from("foo.txt"));
...
fn test_png_image() {
    let db = embedded_mime_db();
    let answer = db.query_filename(OsString::from("foo.png"));
...
fn test_json_file() {
    ...
    let db = embedded_mime_db();
    let answer = db.query_filename(OsString::from("foo.json"));
